...
def run_streaming_preprocessing(input_path, output_path, chunksize=50000):
    ...
    if not os.path.exists(input_path):
        print(
            f"❌ Error: File not found at {input_path}. Place your 6GB file there and name it complaints.csv"
        )
        return
    ...
    for chunk in pd.read_csv(input_path, chunksize=chunksize, low_memory=False):
        ...
        narrative_col = (
            ...
            if "consumer_complaint_narrative" in chunk.columns
        ...
        filtered_chunk["cleaned_narrative"] = filtered_chunk[narrative_col].apply(
            clean_narrative
        )
        filtered_chunk = filtered_chunk[
            filtered_chunk["cleaned_narrative"].str.len() > 10
        ]
    ...
    print(
        f"✨ Finished processing! Saved {total_saved_rows:,} filtered target rows to: {output_path}"
    )


if __name__ == "__main__":
    run_streaming_preprocessing(
        "data/raw/complaints.csv", "data/processed/filtered_complaints.csv"
    )
