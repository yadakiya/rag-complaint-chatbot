...
try:
    ...
    st.error(
        "⚠️ Local Vector Database Index Not Found! Run python src/preprocess.py then python src/embed.py first to create it."
    )
    st.stop()
...
with col2:
    ...
    if "current_contexts" in st.session_state:
        for idx, (ctx, meta) in enumerate(
            ...
            with st.expander(
                f"Source Record #{idx+1} (ID: {meta.get('complaint_id', 'N/A')})"
            ):
                st.caption(
                    f"**Vertical Segment:** {meta.get('product_category', 'General')}"
                )
                st.markdown(f"*{ctx}*")
    else:
        st.write(
            "No active query run logged. Context snippets will appear here dynamically."
        )
